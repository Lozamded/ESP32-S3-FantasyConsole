...
- Un fondo de capa 1 mas chico que la ventana (lo tipico, ya que el asset en si sigue acotado por separado, ver `MAX_BACKGROUND_PIXEL_W/H` en TurtleStudio) simplemente puede no superponerse con la ventana actual si la camara esta lejos de donde esta anclado (esquina inferior-izquierda del mundo) — en ese caso no se hornea nada ahi, como es de esperar (el fondo solo cubre esa parte del mundo).
...
- **Efecto en tiles**: como capa 1 se hornea junto a los tiles en el mismo `s_world_bg` (ver seccion siguiente), una banda con `fixed=true` o `parallax_x != 1.0` re-samplea esa fila al dibujar (`paint_world_background_banded` en `turtle_scene.cpp`) — si esa fila tambien tuviera tiles horneados ahi, el offset de la banda los afectaria a ellos tambien (tiles "pegados" a la camara o desfasados, aunque las bandas deberian tocar solo el fondo). Por eso, en cuanto la escena define **cualquier** `parallax_bands`, el firmware se salta el horneado de tiles por completo (`prepare_world_static_composite`) y los redibuja en vivo cada fotograma con la camara plana — mismo mecanismo (y mismo costo por fotograma) que activar una capa 2-4, ver "Orden de pintado vs. tiles" mas abajo.
- **Muestreo independiente de la ventana residente**: `paint_world_background_banded` NO lee la fila de banda desde la ventana residente (`s_world_bg`, ver "Ventana residente / streaming del mundo" arriba) sino desde `s_bg_decode_scratch` — la decodificacion completa mas reciente de la imagen de capa 1, con columna/fila 0 siempre ancladas a la imagen (nunca al origen movible `s_win_x0/y0` de la ventana). Es necesario porque el offset de banda (`parallax_x * cam_x`, o el `modulo` de `repeat_x`) se calcula en coordenadas de imagen/mundo absolutas; leerlo directo de la ventana producia fondo solido (relleno, "negro") en cuanto la camara se alejaba lo suficiente para que la ventana se recentrara lejos del origen del mundo (~3 pasos de distancia del origen, ver `kWorldWindowSteps`). Filas sin imagen real horneada (solo relleno solido) si usan la ventana, porque ahi cualquier pixel tiene el mismo valor de relleno.
