...
def serialize_tileset_tiles(
    ...
    *,
    ...
    ph: int,
    fill_index: int = 0,
    collision_meta: list[TileCollisionMeta] | None = None,
) -> list[dict[str, Any]]:
    fi = max(0, int(fill_index))
    meta = normalize_tile_collision_meta_list(collision_meta, len(tiles_rows))
    out: list[dict[str, Any]] = []
    for i, rows in enumerate(tiles_rows):
        entry: dict[str, Any] = {
            "image": _pack_tile_image(rows, pw=pw, ph=ph, fill_index=fi)
        }
        coll_field = collision_meta_to_json_field(meta[i])
        if coll_field is not None:
            entry["collision"] = coll_field
        apply_oneway_to_collision_entry(entry, meta[i])
        out.append(entry)
    return out
...
def save_tileset_json(
    ...
    *,
    ...
    merged_meta = collision_meta
    if merged_meta is None and isinstance(previous, dict):
        merged_meta = parse_tileset_collision_meta(previous)
