"""Import-image dialog: pick a PNG/JPG, preview it quantized to the current palette."""
...
class BackgroundImportDialog(QDialog):
    def __init__(
        self,
        project_root: Path,
        pixel_w: int,
        ...
        self.setWindowTitle(tr("background.import_dialog_title"))

        self._file_edit = QLineEdit()
        self._file_edit.setReadOnly(True)
        browse_button = QPushButton(tr("background.import_browse_button"))
        ...
        form = QFormLayout()
        form.addRow(tr("background.import_file_label"), file_row)
        form.addRow(tr("background.import_alpha_cutoff_label"), self._alpha_spin)
    ...
    def _pick_file(self) -> None:
        path, _filter = QFileDialog.getOpenFileName(
            self,
            tr("background.import_dialog_title"),
            "",
            tr("background.import_file_filter"),
        )
        if not path:
            return
        ...
        try:
            ...
            self._rows = None
            self._note_label.setText(tr("background.import_load_error", e=e))
