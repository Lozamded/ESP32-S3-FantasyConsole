"""Import-image dialog for the tileset editor: pick a PNG/JPG, preview it quantized to the
current tileset palette, replace the active tile with the result."""
...
class TilesetImportDialog(QDialog):
    def __init__(
        ...
        *,
        ...
        self.setWindowTitle(tr("tileset.import_dialog_title"))

        self._file_edit = QLineEdit()
        self._file_edit.setReadOnly(True)
        browse_button = QPushButton(tr("tileset.import_browse_button"))
        ...
        form = QFormLayout()
        form.addRow(tr("tileset.import_file_label"), file_row)
        form.addRow(tr("tileset.import_alpha_cutoff_label"), self._alpha_spin)

        self._tile_label = QLabel(tr("tileset.import_tile_note", n=f"T{tile_index}/{tile_count}"))
        self._tile_label.setStyleSheet("color: #888;")
    ...
    def _pick_file(self) -> None:
        path, _filter = QFileDialog.getOpenFileName(
            self,
            tr("tileset.import_dialog_title"),
            "",
            tr("tileset.import_file_filter"),
        )
        if not path:
            return
        ...
        try:
            ...
            self._rows = None
            self._note_label.setText(tr("tileset.import_load_error", e=e))
    ...
    def _refresh_preview(self) -> None:
        ...
        self._rows = convert_ref_source_to_palette_rows(
            self._ref_source, self._tile_px, self._tile_px, self._rgbs01, alpha_cutoff=cutoff
        )
        preview_rgba = composite_sprite_editor_preview(self._rows, self._rgbs01, None)
        image = _rgba_floats_to_qimage(preview_rgba, self._tile_px, self._tile_px)
        ...
        note = aspect_ratio_note(sw, sh, self._tile_px, self._tile_px)
